...
let dummydata = [ //Dummy data used to fill the table(in real scenario data will be taken form the backend with useEffect)
  {
    DRR_Data: "59",
    Last_updated: "2023-10-14 17:23:34",
    Month_Year: "10 - 2023",
    end_date: "2023-10-30",
    excluded_date: ["2023-10-19", "2023-10-21"],
    lead_count: "1000",
    number_Days: "17",
  ...
  {
    DRR_Data: "132",
    Last_updated: "2023-10-15 10:45:29",
    Month_Year: "9 - 2023",
    end_date: "2023-10-28",
    excluded_date: ["2023-10-09", "2023-10-10", "2023-10-11"],
    lead_count: "5000",
    number_Days: "38",
...
const Table = () => {
  ...
  const toggleNew = (e) => { // Show/Hide the input form depending on which button is pressed 
  ...
  const validateNum = (e) => {//check whether the input for Lead count is an integer, else empty input
    if (!Number.isInteger(parseInt(e.target.value))) {e.target.value = "";}
  };

  const setDates = (e) => {// set the values of all the date inputs to states
    ...
    if (e.target.id === "excludedDate") {
      ...
      excluded = `${excluded.getFullYear()}-${String( excluded.getMonth() + 1).padStart(2, "0")}-${String(excluded.getDate()).padStart(2, "0")}`;
      
      if ( dateRegex.test(excluded) && excluded >= minDate && excluded <= maxDate) {//check whether the input value is matching the pattern, and is under the min-max value(necessary when user types the date instead of selecting)
  ...
  const showList = (e)=> {//show the excluded dates list (used primarily for smaller screens like mobile, which will not have hover functionality)
  ...
  const handleexcludeRemove = (date) => {//remove a date from excluded dates
    const newexcudedDate = new Set(excludedDate);
    newexcudedDate.delete(date);
    setexcludedDate(newexcudedDate);
  };

  const handleSaveandUpdate = async (e) => {//get all the data from the inputs and the spans and save as new file or update an already existing file
    e.preventDefault();
    let monthData = document.querySelector(".table-cell.monthdata  span").innerText;
    let NumdaysData = document.querySelector(".table-cell.numdaysdata span").innerText;
    ...
    let saveDate = `${save.getFullYear()}-${String(save.getMonth() + 1).padStart(2, "0")}-${String(save.getDate()).padStart(2, "0")} ${save.getHours()}:${save.getMinutes()}:${save.getSeconds()}`;
    
    const form = new FormData(e.target);// get all the data from inputs
    const formdata = Object.fromEntries(form);

    const totalData = {//put all the data in one variable
      ...formdata,
    ...
    if(!update) {//only save file as new when not updating
      settableData([...tableData, totalData]);//add the new data to the main data
    ...
    else {//code to handle updating the table content
      //updating an already existing file
      let id = parseInt(document.querySelector("#rowID").innerText)
      let newdata = [...tableData]
      newdata[id-1] = totalData
      update = false;
      settableData(newdata)

      //CODE TO UPDATE FILE FROM THE BACKEND BY ID
      // let url = `put_a_valid_URL/api/${Id}`;
      // fetch(url, {
      //   method: 'PUT',
      //   headers: {
      //     'Content-Type': 'application/json',
  ...
  const handleDelete = (index)=> { // we can take the ID instead of index, to make real fetch request
  ...
  const handleUpdate = (rowdata, index)=> {// add values to all the states and the inputs and open the inputbar
    update = true
    toggleNew()
    document.querySelector("#rowID").innerText = index+1
  ...
  useEffect(() => {
    if (startDate && endDate) {// find the difference between start date and end date 
      const difference =
        (endDate - startDate) / (1000 * 3600 * 24) - excludedDate.size + 1;
      setnumDays(difference);
    }
    setminDate(//set min allowed date in correct format
      startDate
    ...
    setmaxDate(//set max allowed date in correct format
      endDate
      ...
      <div className="table">
        ...
        <div className="new-input">
          <form onSubmit={handleSaveandUpdate}>
            ...
            <div className="table-cell" data-label="Start-Date">
              <input id="startDate" onChange={setDates} max={maxDate} type="date" name="start_date" required />
            </div>
            <div className="table-cell" data-label="End-Date">
              <input id="endDate" onChange={setDates} min={minDate} type="date" name="end_date" required />
            </div>
            <div className="table-cell monthdata" data-label="Month-Year">
              <span>{startDate ? `${startDate.getMonth() + 1} - ${startDate.getFullYear()}` : "0"}</span>
            </div>
            <div className="table-cell excluded" data-label="Dates-Excluded">
              <input id="excludedDate" onInput={setDates} min={minDate} max={maxDate} type="date" name="excluded_date" />
              {excludedDate.size > 0 && (
                <>
              <div onClick={showList} className="arrow"><ion-icon name="caret-down-outline"></ion-icon></div>
                <div className="excluded-array">
                  {Array.from(excludedDate).map((date, index) => (
                    <p onClick={() => {handleexcludeRemove(date);}} key={index}>{date}</p>
                  ))}
                </div>
                </>
              )}
            ...
            <div className="table-cell" data-label="Lead-Count">
              <input id="leadCount" required onInput={validateNum} onChange={(e) => {setleadCount(e.target.value);}} type="number" name="lead_count" />
            </div>
            ...
            <div className="table-cell btn-box" data-label="Last-Updated">
              ...
              <ion-icon id="cancel" onClick={toggleNew} name="ban-outline"></ion-icon>
            </div>
          </form>
        </div>
        ...
        {tableData.map((rowdata, index) => (
          <div key={index} className="table-content">
            <div aria-label="Action" className="table-cell" data-label="Action">
              <span> </span>
            </div>
            <div aria-label="ID of current data" className="table-cell" data-label="ID">
              <span>{index + 1}</span>
            </div>
            <div aria-label="Start Date" className="table-cell" data-label="Start Date">
              <span>{rowdata.start_date}</span>
            </div>
            <div aria-label="End Date" className="table-cell" data-label="End Date">
              <span>{rowdata.end_date}</span>
            </div>
            <div aria-label="Month and year" className="table-cell" data-label="Month,Year">
              <span>{rowdata.Month_Year}</span>
            </div>
            <div aria-label="Dates excluded" className="table-cell" data-label="Dates Excluded">
            ...
            <div aria-label="Total number of days" className="table-cell" data-label="Number of Days">
              <span>{rowdata.number_Days}</span>
            </div>
            <div aria-label="Lead count" className="table-cell" data-label="Lead Count">
              <span>{rowdata.lead_count}</span>
            </div>
            <div aria-label="Expected DRR" className="table-cell" data-label="Expected DRR">
              <span>{rowdata.DRR_Data}</span>
            </div>
            <div aria-label="Last Updated" className="table-cell" data-label="Last Updated">
              <span>{rowdata.Last_updated}</span>
            </div>
            <div aria-label="Update this Data" onClick={()=> {handleUpdate(rowdata, index)}} className="btn update">Update</div>
            <div aria-label="Delete this Data" onClick={()=> {handleDelete(index)}} className="btn delete">Delete</div>
          </div>
        ))}
      </div>
    </main>
